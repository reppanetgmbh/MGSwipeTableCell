...
@implementation MGSwipeButtonsView
{
    ...
    CGFloat _panRecognizerOffset;
    BOOL _autoHideExpansion;
}

#pragma mark Layout

-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(MGSwipeDirection) direction differentWidth:(BOOL) differentWidth buttonsDistance:(CGFloat) buttonsDistance panRecognizerOffset:(CGFloat) panRecognizerOffset
{
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, containerWidth, maxSize.height)]) {
        ...
        _panRecognizerOffset = panRecognizerOffset;
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        _leftView = [[MGSwipeButtonsView alloc] initWithButtons:_leftButtons direction:MGSwipeDirectionLeftToRight differentWidth:_allowsButtonsWithDifferentWidth buttonsDistance:_leftSwipeSettings.buttonsDistance panRecognizerOffset:_leftSwipeSettings.panRecognizerOffset];
        _leftView.cell = self;
    ...
    if (!_rightView && _rightButtons.count > 0) {
        _rightView = [[MGSwipeButtonsView alloc] initWithButtons:_rightButtons direction:MGSwipeDirectionRightToLeft differentWidth:_allowsButtonsWithDifferentWidth buttonsDistance:_rightSwipeSettings.buttonsDistance panRecognizerOffset:_leftSwipeSettings.panRecognizerOffset];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    BOOL leftOffsetOk = location.x >= _leftSwipeSettings.panRecognizerOffset;
    BOOL rightOffsetOk = location.x < (gestureRecognizer.view.frame.size.width - _rightSwipeSettings.panRecognizerOffset);
    return leftOffsetOk && rightOffsetOk;
}
